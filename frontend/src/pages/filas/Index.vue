    ...
    >
      <template v-slot:top>
        <div class="row items-center justify-between full-width">
          <div>
            <div class="tw-text-lg tw-font-bold tw-text-foreground">Setores</div>
            <div class="tw-text-sm tw-text-muted-foreground">Departamentos que organizam o roteamento dos atendimentos</div>
          </div>
          <q-btn
            color="primary"
            label="Adicionar"
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'queue', label: 'Setor', field: 'queue', align: 'left' },
  ...
  methods: {
    ...
    deletarFila (fila) {
      this.$q.dialog({
        title: 'Atenção!!',
        message: `Deseja realmente deletar o Setor "${fila.queue}"?`,
        cancel: {
          label: 'Não',
          color: 'primary',
          push: true
        ...
        DeletarFila(fila)
          .then(res => {
            ...
            this.$q.notify({
              ...
              progress: true,
              position: 'top',
              message: `Setor ${fila.queue} deletado!`,
              actions: [{
                icon: 'close',
                round: true,
                color: 'white'
