<template>
  ...
  >
    <q-toolbar class="text-grey-8 flow-toolbar">
      <q-icon
        name="mdi-robot-outline"
        size="22px"
        class="tw-text-primary q-mr-sm"
      />
      <q-toolbar-title>
      ...
      >
        <q-tooltip>Excluir elemento selecionado</q-tooltip>
      </q-btn>
      <q-separator
        inset
        spaced
    ...
    >
      ...
      <div
        ...
        style="width: 560px; border-left: 1px solid hsl(214 32% 91%); overflow-y: auto; height: 100%;"
      >
        <flow-node-form
          ref="nodeForm"
  ...
  methods: {
    ...
    addNewLineCondition (from, to, oldTo) {
      ...
      const connParam = {
        source: from,
        target: to,
        paintStyle: { strokeWidth: 3, stroke: '#FF6584' }
      }
    ...
    setLineLabel (from, to, label) {
      ...
      conn.setPaintStyle({ strokeWidth: 3, stroke: '#FF6584' })
...
.flow-config-panel
  background: hsl(210 40% 98% / .4)

body.body--dark .flow-toolbar
  border-bottom: 1px solid #262626

body.body--dark .flow-config-panel
  background: #0A0A0A
</style>
